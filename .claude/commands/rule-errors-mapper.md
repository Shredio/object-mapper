Run command `vendor/bin/phpunit tests/Unit/Rule/ObjectMapperRuleTest.php` and fix the analyse errors in the second argument of `$this->analyse(...)` method in test file so that the test passes.
